...
public class RutaAE {
    ...
    public void visitarCiudad(NodoAE actual){
        ...
        for(String subCiudad: subMapa){             
            ...
            hijo.setG_n(sumCostos + actual.getG_n()); //calcula el coste acumulado
            hijo.setF_n(getHeuristica(hijo));
            actual.addHijo(hijo);            
            if(checkVisitados(visitados,hijo)){ //Si nodo hijo no esta en visitados
                if(checkFrontera(Frontera,hijo)){ //Si nodo hijo esta en frontera
                    if(hijo.getF_n() < getNodoFrontera(hijo.getDatos()).getF_n()){
...
//                        Frontera.set(getIndexFrontera(hijo.getDatos()), hijo);
    ...
    /*Función heurística, da la distancia en km de un punto a otro*/
    ...
    public int getHeuristica(NodoAE nodo){
        Double lat1, lat2, lon1, lon2;
        int d;
        lat1 = coordenadas.get(nodo.getDatos())[0];
        lon1 = coordenadas.get(nodo.getDatos())[1];
        lat2 = coordenadas.get(control.getCiudadFinal())[0];
        lon2 = coordenadas.get(control.getCiudadFinal())[1];
        d = distGeodesica(lat1,lon1,lat2,lon2).intValue();
        return nodo.getG_n() + d;
    ...
    private void initCoordenadas(){
        Double[] coordCDMX = {19.25,99.13}; //Latitud, Longitud
        Double[] coordToluca = {19.28,99.66};
        Double[] coordCuerna = {18.92,99.22};
        Double[] coordPuebla = {19.04,98.21};
        Double[] coordPachuca = {20.10,98.76};
        Double[] coordQueretaro = {20.56,100.39};
        Double[] coordPoza = {20.53,97.46};
        Double[] coordAca = {16.85,99.82};
        Double[] coordVera = {19.17,96.13};
        Double[] coordHuatulco = {15.83, 96.31};
        coordenadas.put("CDMX",coordCDMX);
        coordenadas.put("Toluca",coordToluca);
        coordenadas.put("Cuernavaca",coordCuerna);
        coordenadas.put("Puebla",coordPuebla);
        coordenadas.put("Pachuca",coordPachuca);
        coordenadas.put("Querétaro",coordQueretaro);
        coordenadas.put("Poza Rica",coordPoza);
        coordenadas.put("Acapulco",coordAca);
        coordenadas.put("Veracruz",coordVera);
        coordenadas.put("Huatulco",coordHuatulco);
    }
